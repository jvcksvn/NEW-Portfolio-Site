...
const jobs=[
  ...
  ['mobile-full','/',390,844,true],
  ['menu-open','/',390,844,false],
  ['404','/404',1280,700,false],
  ['500','/500',1280,700,false],
];
for(const [name,path,w,h,full] of jobs){
  ...
  if(name==='menu-open'){await p.locator('.mobile .trigger').click();await p.waitForTimeout(500);}
  await p.screenshot({path:`/tmp/s-${name}.png`,fullPage:full});
  console.log(name,'ok');
  await ctx.close();
}
